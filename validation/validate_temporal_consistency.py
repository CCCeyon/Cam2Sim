...
from utils.validation import compute_ssim, compute_psnr, compute_mse, compute_cpl, get_segmentation_model, \
    calculate_semantic_segmentation_score
...
sim_folders = [f.path for f in os.scandir(output_dir) if
               f.is_dir() and f.name not in ["seg", "canny", "depth", "old"]]
csv_filename = os.path.join(output_dir, "results_tempconsistency.csv")
...
for simulated_folder in tqdm(natsorted(sim_folders), desc="Processing Folders"):
    local_csv_filename = os.path.join(simulated_folder, "results_tempconsistency.csv")
    if show_messages:
    ...
    first_sim_path = os.path.join(simulated_folder, sim_images[0])
    first_sim_image = cv2.imread(first_sim_path)
    _, prev_seg_image = calculate_semantic_segmentation_score(seg_model, seg_image_extractor, None,
                                                                   first_sim_image, carla_mode=carla_mode)

    for i in tqdm(range(num_pairs), desc=f"Frames for {simulated_folder}"):
        sim_path = os.path.join(simulated_folder, sim_images[i + 1])
        sim_path2 = os.path.join(simulated_folder, sim_images[i])

        # Read images using cv2 (which loads as BGR)
        sim_image = cv2.imread(sim_path)
        sim_image2 = cv2.imread(sim_path2)
        ...
        if sim_path2 is None:
            print(f"Could not load sim2 image: {sim_path2}. Skipping.")
            continue
        ...
        ssim_score = compute_ssim(sim_image, sim_image2)
        psnr_score = compute_psnr(sim_image, sim_image2)
        mse_score = compute_mse(sim_image, sim_image2)
        semantic_seg_score, prev_seg_image = -1, None #calculate_semantic_segmentation_score(seg_model, seg_image_extractor, prev_seg_image, sim_image, carla_mode=carla_mode)
        # semantic_seg_score = 0.0

        # For CPL, convert images to RGB
        if args.cpl:
            sim_image_rgb = cv2.cvtColor(sim_image, cv2.COLOR_BGR2RGB)
            sim_image2_rgb = cv2.cvtColor(sim_image2, cv2.COLOR_BGR2RGB)
            cpl_score = compute_cpl(sim_image_rgb, sim_image2_rgb, vgg16, transform)
        else:
            cpl_score = -1

        # check if psnr_score is not inf
        if psnr_score != float('inf') and mse_score != 0.0:
        ...
        results.append([os.path.basename(sim_path), os.path.basename(sim_path2),
                        ssim_score, psnr_score, mse_score, cpl_score, semantic_seg_score])

        if show_messages:
            print(f"Pair {i + 1}: {os.path.basename(sim_path)} vs {os.path.basename(sim_path2)}")
            print(
